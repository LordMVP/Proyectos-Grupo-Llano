package com.gell.estandar.persistencia.util;
...
public class PreparedStatementNamed
{
   ...
   */
  public PreparedStatementNamed(Connection connection, String query, boolean returnGeneratedKeys)
          throws SQLException
  {
  ...
  public PreparedStatementNamed(Connection connection, String query)
          throws SQLException
  {
   ...
   * CODE. This method is non-private so JUnit code can testSelect it.
   *
          ...
          throws SQLException
  {
    ...
    for (int i = 0; i < indexes.length; i++) {
      if (value instanceof java.util.Date) {
        statement.setObject(indexes[i], DateUtil.parseTimestamp((java.util.Date) value));
        continue;
      }
   ...
   * inputStream)
   */
  public void setInputStream(String name, InputStream value)
          throws SQLException
  {
    int[] indexes = getIndexes(name);
    for (int i = 0; i < indexes.length; i++) {
