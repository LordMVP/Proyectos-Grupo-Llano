...
import com.bioagricola.apirest.modelo.manejadores.ManejadorRecRecaudo;
...
/**
 * Servicios para operaciones CRUD y de negocio sobre la entidad RecRecaudo
 * @author GeneradorCRUD
 */
@Service
public class NegocioRecRecaudo extends NegocioAbstracto<RecRecaudo,RecRecaudoDTO> {

    @Autowired
    private ManejadorRecRecaudo manejadorRecRecaudo;

    /**
     * Variable estatica para imprimir logs...
     */
    private static final Logger logger = Logger.getLogger(NegocioRecRecaudo.class.getName());
     ...
     */
    public List<RecRecaudoDTO> consultar(String filterBy, 
        ...
        return convertirListaEntidadesADao(manejadorRecRecaudo.consultar(filtros, ordenamiento, rango));
     ...
     * @return La insntancia de RecRecaudo recién creado
     */
    public RecRecaudoDTO crear(RecRecaudoDTO recRecaudoDTO) {
    	// protected region Modifique el metodo crear on begin
    	
        logService(this.getClass().getName(), "crear", recRecaudoDTO);

        RecRecaudo recRecaudo = new RecRecaudo();
        copiarPropiedades(recRecaudo, recRecaudoDTO);
        
        manejadorRecRecaudo.save(recRecaudo);

        return recRecaudoDTO;
     ...
     * @return La instancia de la entidad RecRecaudo que ha sido actualizado
     */
    public RecRecaudoDTO actualizar(RecRecaudoDTO recRecaudoDTO){
        // protected region Modifique el metodo actualizar on begin
    
        logService(this.getClass().getName(), "actualizar", recRecaudoDTO);

        RecRecaudo recRecaudo = manejadorRecRecaudo.getOne(recRecaudoDTO.getRecIderegistro());                
        copiarPropiedades(recRecaudo, recRecaudoDTO);
    ...
    public String eliminar(Long recIderegistro) {
        ...
        logService(this.getClass().getName(), "eliminar", recIderegistro);
        manejadorRecRecaudo.deleteById(recIderegistro);
        
		
		StringBuilder valores = new StringBuilder();
		valores.append(String.valueOf(recIderegistro));
    ...
    public String contar(String filterBy,
        ...
        return String.valueOf(manejadorRecRecaudo.consultarTotalRegistros(filtros, 
                    rango));
    ...
    public List<String> consultarLista(String filterBy,
        ...
        return UtilOperaciones.convertirListaObjetosAString(manejadorRecRecaudo.consultarLista(filtros, ordenamiento, infoAgrupamiento));
    ...
    protected boolean entidadContieneAtributo(String nombreAtributo) {
        return RecRecaudo.contieneAtributo(nombreAtributo);
    }
     ...
     */
    @Override
    protected RecRecaudoDTO instanciarDAO() {
        return new RecRecaudoDTO();
    }       
