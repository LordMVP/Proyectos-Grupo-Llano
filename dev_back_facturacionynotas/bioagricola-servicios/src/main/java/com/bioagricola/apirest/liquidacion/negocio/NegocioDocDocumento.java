...
/**
 * Servicios para operaciones CRUD y de negocio sobre la entidad DocDocumento
 * @author GeneradorCRUD
 */
@Service
public class NegocioDocDocumento extends NegocioAbstracto<DocDocumento,DocDocumentoDTO> {
     ...
     */
    private static final Logger logger = Logger.getLogger(NegocioDocDocumento.class.getName());
    ...
    /**
     * Realiza un consulta en la entidad DocDocumento aplicando los filtros, el ordenamiento,
     ...
     * Ej. Una secuencia de parámetros de filtrado puede ser {@literal docDocumentoId>1&docDocumentoName:LIKE:juan}
     ...
     * Ej. Una secuencia de parámetros de ordenamiento puede ser: docDocumentoId$ASC, docDocumentoName$DESC
     ...
     * @return Una lista de DAOs de los DocDocumento que se consultaron con los parámetros enviados por el cliente
     ...
     */
    public List<DocDocumentoDTO> consultar(String filterBy, 
        ...
        return convertirListaEntidadesADao(manejadorDocDocumento.consultar(filtros, ordenamiento, rango));
        // protected region Modifique el metodo consultar end
    }

    /**
     * Crea el docDocumento que se pasa como parámetro en la base de datos.
     * 
     * @param docDocumentoDTO El DAO de la entidad DocDocumento a crear. Este se envía en el cuerpo de la
     * solicitud POST como un objeto JSON.
     * @return La insntancia de DocDocumento recién creado
     */
    public DocDocumentoDTO crear(DocDocumentoDTO docDocumentoDTO) {
    	// protected region Modifique el metodo crear on begin
    	
        logService(this.getClass().getName(), "crear", docDocumentoDTO);

        DocDocumento docDocumento = new DocDocumento();
        copiarPropiedades(docDocumento, docDocumentoDTO);
        
        manejadorDocDocumento.save(docDocumento);

        return docDocumentoDTO;
        // protected region Modifique el metodo crear end
    }

    /**
     * Actualiza en la base de datos el docDocumento que se pasa como parámetro.
     * 
     * @param docDocumentoDTO El DAO de la entidad DocDocumento a actualizar. Este se envía en el cuerpo de la
     * solicitud PUT como un objeto JSON.
     * @return La instancia de la entidad DocDocumento que ha sido actualizado
     */
    public DocDocumentoDTO actualizar(DocDocumentoDTO docDocumentoDTO){
        ...
        logService(this.getClass().getName(), "actualizar", docDocumentoDTO);

        DocDocumento docDocumento = manejadorDocDocumento.getOne(docDocumentoDTO.getUniDocumento());                
        copiarPropiedades(docDocumento, docDocumentoDTO);
    ...
    /**
     * Elimina el docDocumento con el identificador que se pasa como parámetro.
     * 
     * @param uniDocumento Valor del atributo del identificador de la instancia de la entidad  docDocumento a eliminar
     * @return El identificador del docDocumento que ha sido eliminado
     */
    public String eliminar(Integer uniDocumento) {
        // protected region Modifique el metodo eliminar on begin

        logService(this.getClass().getName(), "eliminar", uniDocumento);
        manejadorDocDocumento.deleteById(uniDocumento);
        
		
		StringBuilder valores = new StringBuilder();
		valores.append(String.valueOf(uniDocumento));
    ...
    public String contar(String filterBy,
        ...
        return String.valueOf(manejadorDocDocumento.consultarTotalRegistros(filtros, 
                    rango));
     ...
     * parámetro debe ir separado por coma : ','.
     * Ej. Una secuencia de parámetros de ordenamiento puede ser: docDocumentoId$ASC, docDocumentoName$DESC
     * @param atributo Nombre del atributo de la entidad DocDocumento del cual se quieren obtener los diferentes valores.
    ...
    public List<String> consultarLista(String filterBy,
        ...
        return UtilOperaciones.convertirListaObjetosAString(manejadorDocDocumento.consultarLista(filtros, ordenamiento, infoAgrupamiento));
        // protected region Modifique el metodo consultarLista end
    }
    
    /**
     * Método encargado de manejar el retorno de la consulta y mapearlo a un DTO de respuesta para
     * el servicio de consulta de documentos
     * */
    public List<DocumentoDTO> consultaDocumentos() {
    	...
    	List<Object[]> listaEntidadDocumentos = manejadorDocDocumento.consultaDocumentos(idEmpresa);
    	List<DocumentoDTO> listaDocumentoDTO = new ArrayList<>();
    	
    	//Mapeo de los objetos de respuesta de la consulta al DTO de respuesta del servicio
    	for (Object[] row : listaEntidadDocumentos) {
			DocumentoDTO documento = new DocumentoDTO();
			documento.setUniDocumento((Integer)row[0]);
			documento.setUniNombre1((String)row[1]);
    ...
    protected boolean entidadContieneAtributo(String nombreAtributo) {
        return DocDocumento.contieneAtributo(nombreAtributo);
    }
     ...
     */
    @Override
    protected DocDocumentoDTO instanciarDAO() {
        return new DocDocumentoDTO();
    }
