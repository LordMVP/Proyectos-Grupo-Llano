...
import com.bioagricola.apirest.modelo.dtos.EmpresasHomologadasDTO;
import com.bioagricola.apirest.modelo.entidades.Empresas;
import com.bioagricola.apirest.modelo.excepciones.InvalidParameterException;
import com.bioagricola.apirest.modelo.manejadores.ManejadorEmpresas;
...
/**
 * Servicios para operaciones CRUD y de negocio sobre la entidad Empresas
 * 
 * @author GeneradorCRUD
 */
@Service
public class NegocioEmpresas extends NegocioAbstracto<Empresas, EmpresasDTO> {

	@Autowired
	private ManejadorEmpresas manejadorEmpresas;

	/**
	 * Variable estatica para imprimir logs...
	 */
	private static final Logger logger = Logger.getLogger(NegocioEmpresas.class.getName());
	...
	/**
	 * Realiza un consulta en la entidad Empresas aplicando los filtros, el
	 ...
	 *                 {@literal empresasId>1&empresasName:LIKE:juan}
	 ...
	 *                 ser: empresasId$ASC, empresasName$DESC
	 ...
	 * @return Una lista de DAOs de los Empresas que se consultaron con los
	...
	public List<EmpresasDTO> consultar(String filterBy, String orderBy, Integer from, Integer to)
		...
		return convertirListaEntidadesADao(manejadorEmpresas.consultar(filtros, ordenamiento, rango));
		// protected region Modifique el metodo consultar end
	}

	/**
	 * Crea el empresas que se pasa como parámetro en la base de datos.
	 * 
	 * @param empresasDTO El DAO de la entidad Empresas a crear. Este se envía en el
	 *                    cuerpo de la solicitud POST como un objeto JSON.
	 * @return La insntancia de Empresas recién creado
	 */
	public EmpresasDTO crear(EmpresasDTO empresasDTO) {
		// protected region Modifique el metodo crear on begin

		logService(this.getClass().getName(), "crear", empresasDTO);

		Empresas empresas = new Empresas();
		copiarPropiedades(empresas, empresasDTO);

		manejadorEmpresas.save(empresas);
	...
	/**
	 * Actualiza en la base de datos el empresas que se pasa como parámetro.
	 * 
	 * @param empresasDTO El DAO de la entidad Empresas a actualizar. Este se envía
	 *                    en el cuerpo de la solicitud PUT como un objeto JSON.
	 * @return La instancia de la entidad Empresas que ha sido actualizado
	 */
	public EmpresasDTO actualizar(EmpresasDTO empresasDTO) {
		// protected region Modifique el metodo actualizar on begin

		logService(this.getClass().getName(), "actualizar", empresasDTO);

		Empresas empresas = manejadorEmpresas.getOne(empresasDTO.getEmpresaCod());
		copiarPropiedades(empresas, empresasDTO);
	...
	/**
	 * Elimina el empresas con el identificador que se pasa como parámetro.
	 * 
	 * @param empresaCod Valor del atributo del identificador de la instancia de la
	 *                   entidad empresas a eliminar
	 * @return El identificador del empresas que ha sido eliminado
	 */
	public String eliminar(String empresaCod) {
		// protected region Modifique el metodo eliminar on begin

		logService(this.getClass().getName(), "eliminar", empresaCod);
		manejadorEmpresas.deleteById(empresaCod);

		StringBuilder valores = new StringBuilder();
		valores.append(String.valueOf(empresaCod));
	...
	public String contar(String filterBy, Integer from, Integer to) throws InvalidParameterException {
		...
		return String.valueOf(manejadorEmpresas.consultarTotalRegistros(filtros, rango));
	 ...
	 *                 ','. Ej. Una secuencia de parámetros de ordenamiento puede
	 *                 ser: empresasId$ASC, empresasName$DESC
	 * @param atributo Nombre del atributo de la entidad Empresas del cual se
	...
	public List<String> consultarLista(String filterBy, String orderBy, String atributo)
		...
		return UtilOperaciones.convertirListaObjetosAString(
				manejadorEmpresas.consultarLista(filtros, ordenamiento, infoAgrupamiento));
	...
	protected boolean entidadContieneAtributo(String nombreAtributo) {
		return Empresas.contieneAtributo(nombreAtributo);
	}
	 ...
	 */
	@Override
	protected EmpresasDTO instanciarDAO() {
		return new EmpresasDTO();
	}

	public Object getInfoSesion(Integer idacceso) {

		return manejadorEmpresas.getInfoSesion(idacceso);
	...
	public List<EmpresasHomologadasDTO> consultaEmpresasHomologadas() {

		int empresaId = JwtUtil.auditoriaDTO.getIdEmpresa();
		List<Object[]> listaResultados ;

		listaResultados = manejadorEmpresas.consultaHomologadas(empresaId);

		List<EmpresasHomologadasDTO> listaEmpresas = new ArrayList<>();

		for (Object[] row : listaResultados) {
			EmpresasHomologadasDTO resposeEmpresasHomologadas = new EmpresasHomologadasDTO();
			resposeEmpresasHomologadas.setEmpresaCod((String) row[0]);
			resposeEmpresasHomologadas.setEmpresaNom((String) row[1]);
			resposeEmpresasHomologadas.setEmpIderegistro((Integer) row[2]);
			if (!(resposeEmpresasHomologadas.getEmpIderegistro().equals(empresaId))) {
				listaEmpresas.add(resposeEmpresasHomologadas);
			}
		}
