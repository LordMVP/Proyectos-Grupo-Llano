...
import './GestionActividadesColaborador.scss';
...
class GestionActividadesColaborador extends Component {
  ...
  buscar = () => {
    axios.post(RUTAS_API.REPORTE_ACTIVIDADES_COLABORADOR.CONSULTAR_ACTIVIDADES_COLABORADOR,
      {
  ...
  ordenarFechas = (listaFechas) => {
    let fechas = listaFechas.map(fecha => {
      return (new Date(fecha.eje_fechejec)).getTime();
    });
    let listaFinal = [];
    fechas = fechas.sort();
    fechas.forEach((fecha, index) => {
      const item = listaFechas.find(f => {
        if ((new Date(f.eje_fechejec)).getTime() == fecha) {
          return f;
        }
      });
      if (item != null) {
  ...
  renderTablaActividadesColaborador = () => {
    ...
    lista.forEach((item, index) => {
      ...
      valores.forEach(valor => {
        const indice = listaFechas.findIndex(f => f.eje_fechejec == valor.eje_fechejec);
        if (indice < 0) {
          listaFechas.push(valor);
        }
      });
    });
            ...
            <tr>
              ...
              <th>Colaborador</th>
              {
                listaFechas.map(fecha => {
                  return <th>{this.parsearFecha(fecha.eje_fechejec)}</th>
                })
              }
              <th>Total</th>
            </tr>
          ...
          <tbody>
            {
              lista.map((distribucion, index) => {
                const valores = JSON.parse(distribucion.valores);
                let valorTotalLinea = 0;
                return (
                  <Fragment>
                    <tr key={distribucion.cedula}>
                      ...
                      <td>{distribucion.nombre}</td>
                      {
                        listaFechas.map((fecha, index) => {
                          let indice = valores.findIndex(f => f.eje_fechejec == fecha.eje_fechejec);
                          let valor = valores[indice];
                          valor = (valor) ? valor.valor : 0;
                          valorTotalLinea += valor;

                          if (!valoresTotales[index]) {
                            valoresTotales[index] = valor;
                          } else {
                            valoresTotales[index] += valor;
                          }

                          if (index == (listaFechas.length - 1)) {
                            if (!valoresTotales[index + 1]) {
                              valoresTotales[index + 1] = valorTotalLinea;
                            } else {
                              valoresTotales[index + 1] += valorTotalLinea;
                            }
                          }
                          return <td>{formatterPeso.format(valor)}</td>;
                        })
                      }
                      <td>{formatterPeso.format(valorTotalLinea)}</td>
                    </tr>
                    {
  ...
  render() {
    return (
      <Fragment>
        <h1>Reporte Actividades Colaborador</h1>
...
GestionActividadesColaborador.propTypes = {
...
const VistaRedux = connect(mapStateToProps, mapDispatchToProps)(GestionActividadesColaborador);

export { VistaRedux as RGestionActividadesColaborador };
