...
import {
  mostrarCargador,
  ocultarAlerta,
  ocultarProgramaModal,
} from './store/actions/AplicacionAcciones'
...
class App extends Component {
    ...
    axios.interceptors.request.use(
      (axiosConfig) => {
        if (this.programaActual) {
          const idPrograma = this.programaActual.prgIderegistro
            .prgIderegistro
  ...
  consultarSesion = () => {
    axios
      .post(RUTAS_API.PARAMETRIZACION.GESTION_MENU.OBTENER_SESION, {
        criterio: '',
      })
      .then((respuesta) => {
  ...
  componentDidMount() {
    if (config.cargarMenuReal) {
      axios(RUTAS_API.MENU.CONSULTAR_MENU, null).then((respuesta) => {
        if (respuesta.data.codigo > 0) {
          const datos = respuesta.data.datos
          this.setState({ opcionesMenu: datos, programas: datos })
        }
      })
      return
    }
    ...
    if (!Util.validarArreglo(botones)) {
      return (
        <button
          ...
          onClick={() => contexto.props.ocultarAlerta()}>
          Aceptar
                </button>
      )
    }
  ...
  obtenerInformacionComponente = (componente) => {
    ...
    for (const key in RUTAS_VISTA) {
      ...
      if (
        ...
        componente &&
        componente.type &&
        componente.type.WrappedComponent
      ) {
        if (wrap.name === componente.type.WrappedComponent.name) {
          const ruta = RUTAS_VISTA[key].url.substring(1)
          const index = this.props.programas.findIndex((p) =>
            p.prgIderegistro.prgLocaliza.endsWith(ruta)
          )
          if (index !== -1) {
            permitido = true
    ...
    if (!infoComponente.permitido) {
      componenteRenderizar = (
        <div className="text-center mt-5">
          Usted no tiene permisos para acceder a este programa o
          funcionalidad.
                </div>
      )
    }
  ...
  obtenerTituloPrograma = () => {
    ...
    programaSeleccionado = this.obtenerPrograma(programas, ruta)
  ...
  renderLayout = () => {
    if (
      !Util.validarArreglo(this.state.opcionesMenu) ||
      !this.state.sesion
    ) {
      return null
    }
    ...
    if (this.props.esAutenticado) {
      rutas = (
        <Switch>
          <Route
            path={RUTAS_VISTA.RAIZ.url}
            exact
            component={RUTAS_VISTA.RAIZ.componente}
            titulo="Index"
          />
          {this.obtenerProgramasPermitidos()}
          <Route
            component={RUTAS_VISTA.VISTA_NO_PERMITIDA.componente}
          />
        </Switch>
      )
    }
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators(
    { mostrarCargador, ocultarAlerta, ocultarProgramaModal },
    dispatch
  )
