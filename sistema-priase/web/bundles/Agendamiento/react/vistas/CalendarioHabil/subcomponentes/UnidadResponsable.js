...
class UnidadResponsable extends Component {
  ...
  agregar = () => {
    let seleccionados = this.obtenerListaSeleccionados();
    let validar = 0

    this.props.lista.some(function(id){
       seleccionados.some(function(sel){
          if(id.idUnidadResponsable.split('-')[0].trim()===sel.id.split('-')[0].trim()){
            validar = id.idUnidadResponsable.split('-')[0].trim()
            return true;
          } 
       })
    })
    ...
    if(validar>0){
      this.setState({
        titulo: '¡YA ESTA ASOCIADO LAS URE!',
        texto: `Revisar la URE, `+validar + ` ya se encuentra asociada. `,
      })   
      this.setState({ mostrar: true })
    }
    else{
        this.props.lista.forEach(i => {
          seleccionados.push({
            idUnidadResponsable:i.idUnidadResponsable,
            seleccionado: true,
            id:i.idUnidadResponsable,
            texto:i.idUnidadResponsable
          })
        })
