...
class Menu extends Component {
    ...
    cargarListaProgramas = (opcion, arregloProgramas) => {
        ...
        opcion.menuItem.forEach((submenu) =>
            this.cargarListaProgramas(submenu, arregloProgramas)
        )
    }
    ...
    renderItemMenu = (opcion) => {
        ...
        if (
            !opcion.menuItem &&
            (opcion.prgIderegistro && opcion.prgIderegistro.prgLocaliza)
        ) {
            if (opcion.visible === false) {
                return null
            }
            ...
            let link = null
            if (opcion.opcTipo === TIPOS_MENU.AGAU) {
                let rutaPrograma = opcion.prgIderegistro.prgLocaliza
                rutaPrograma = rutaPrograma.substring(
                    rutaPrograma.lastIndexOf('/')
                )
                link = (
                    <Link to={rutaPrograma} title={opcion.opcDescripcion}>
                        {opcion.opcNombre}
                    </Link>
                )
            } else {
                link = (
                    <a
                        href={opcion.prgIderegistro.prgLocaliza}
        ...
        const opcionesHijas = opcion.menuItem.map((opcionHija) =>
            this.renderItemMenu(opcionHija)
        )
        const atributosItem = {
        ...
        return (
            <li {...atributosItem}>
                ...
                <ul
                    className={
                        'menu__subitem' + (this.state.filtrando ? ' open' : '')
                    }>
                    {opcionesHijas}
                </ul>
            </li>
    ...
    onChangeFiltro = (evento) => {
        const filtrando =
            evento.currentTarget.value.trim() !== '' ? true : false
        this.setState({
            filtro: evento.currentTarget.value.trim(),
            filtrando: filtrando,
        })
    }
     ...
     * @return {boolean}
     */
    comprarCadenas = (cadena1, cadena2) => {
        cadena1 = cadena1
            .toLowerCase()
            .replace(/\s+/g, ' ')
            .normalize('NFD')
    ...
    crearMenuTemporal = (q, opciones) => {
        ...
        for (let i = 0; i < opciones.length; i++) {
            ...
            if (this.comprarCadenas(nodoRaiz.opcNombre, q)) {
                temp.push(nodoRaiz)
                continue
            }
    ...
    actualizarEstadoVisible = (filtro, items, opcionActual) => {
        ...
        items.forEach((opcion) => {
            ...
            if (
                opcion.opcNombre &&
                opcion.opcNombre.toUpperCase().indexOf(filtro) === -1
            ) {
