...
class App extends Component {
  ...
  componentDidMount() {
    if (config.cargarMenuReal) {
      axios.post(RUTAS_API.MENU.CONSULTAR_MENU, null).then((respuesta) => {
  ...
  mostrarToast = (respuesta) => {
    ...
    /* TODO: 12/2/2021 Hay que aclarar si se manejarán códigos de respuesta en los servicios para descomentar esta validación */
  ...
  renderAlerta = () => {
    ...
    return (
      <Modal visible={mostrar}>
        ...
        <div className="modal-footer">{this.renderBotones(botones)}</div>
      </Modal>
    );
  };
    ...
    if (!infoComponente.permitido) {
      componenteRenderizar = (
        <div className="text-center mt-5">
          Usted no tiene permisos para acceder a este programa o funcionalidad.
        </div>
      );
    }
