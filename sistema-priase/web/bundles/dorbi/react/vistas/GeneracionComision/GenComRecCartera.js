...
class GenComRecCartera extends Component{
  state = {
    consulta: {
  ...
  consultarComision = () => {
    ...
    const ter_ideregistro = consulta.ter_ideregistro ? consulta.ter_ideregistro : 0;
    const uni_medpago = consulta.uni_medpago ? consulta.uni_medpago : 0;
    const bcu_ideregistro = consulta.bcu_ideregistro ? consulta.bcu_ideregistro : 0;
    const pcrc_vigencia_desde = consulta.fecha_desde;
    const pcrc_vigencia_hasta = consulta.fecha_hasta;
    const limit = 10000;
    ...
    axios.post(URL_GEN_COM+RUTAS_API.GEN_COMISION.CARTERA+'/consulta', data, { params: { limit } })
      .then(respuesta => this.setState({ comisiones: respuesta.data.datos },
  ...
  procesarComision = () => {
    ...
    axios.post(URL_GEN_COM+RUTAS_API.GEN_COMISION.CARTERA+'/'+periodo.id, data)
      .then()
      .catch((error) => console.log(error));
  }
  ...
  obtenerTipos = () => {
    axios.get(URL_GEN_COM+RUTAS_API.GEN_COMISION.NOV_TIPOS)
      .then(respuesta => this.setState({ nov_tipos: respuesta.data.datos }))
      .catch((error) => console.log(error));
  }

  obtenerPeriodo = () => {
    axios.get(URL_GEN_COM+RUTAS_API.GEN_COMISION.PERIODO_CARTERA)
      .then(respuesta => {this.setState({ periodo: {
  ...
  cierrePeriodo = (evento) => {
    ...
    axios.put(URL_GEN_COM+RUTAS_API.GEN_COMISION.PERIODO_CARTERA_CERRAR)
      .then()
      .catch((error) => console.log(error));
  }
  ...
  renderizarPaginas = () => {
    ...
    const filas = slices.map((dato, index) =>
      <tr key={`rec-${index}`}>
        <td>{dato.pcrc_parcomrecart.terceroObject.ter_nomcompleto}</td>
        <td>{dato.pcrc_parcomrecart.bcu_ideregistro_object.bcu_numcuenta}</td>
        <td>{dato.pcrc_parcomrecart.medpagoObject.uni_nombre1}</td>
        <td>{moment(dato.gcc_fecharecaudo).format('DD/MM/YYYY')}</td>
        <td>{dato.gcc_dias}</td>
        <td align="right">{new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' })
          .format(dato.gcc_valorbase)}</td>
        <td align="right">{dato.gcc_cantidad}</td>
        <td align="right">{new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' })
          .format(dato.gcc_comisinimpuesto)}</td>
        <td align="right">
          <span className="mr-2">{new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' })
            .format((dato.gcc_comisinimpuesto/100)
            *dato.gcc_comiconimpuesto)}</span>
          <button
        ...
        <td align="right">{new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' })
          .format(dato.gcc_comisinimpuesto -
          (dato.gcc_comisinimpuesto/100)*
          dato.gcc_comiconimpuesto)}</td>
        <td>
          <button
            className="btn btn-primary"
            onClick={() => this.controlarModal(
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        <div>
          <span className="mr-2">{periodo.ciclo} - {periodo.periodo}</span>
          <button
            className="btn btn-primary"
            onClick={this.cierrePeriodo}>Cierre</button>
        </div>
          ...
          <table className='table mt-8'>
            <thead>
              <tr>
                ...
                <th scope="col">Edad Cartera</th>
                <th scope="col">Valor Base</th>
                <th scope="col">Cantidad de<br/>Transacciones</th>
                <th scope="col">Comisión sin<br/>Impuestos</th>
                <th scope="col">Valor de<br/>Impuestos</th>
        ...
        {pageData.map((data, index) =>
          [<Modal
            ...
            onClickBackdrop={() => this.controlarModal(
              index, 'mostrarModalImpuesto')}>
            <RGenComImpuestos
              impuestos={data.pcrc_parcomrecart.impuesto}
              index={index}
...
const VistaRedux = connect(mapStateToProps, mapDispatchToProps)(GenComRecCartera);

export { VistaRedux as RGenComRecCartera };