...
import './GestionCargarVariablesPeriodos.scss';
...
class GestionCargarVariablesPeriodos extends Component {
  ...
  construirObjetoPeriodos = (periodos) => {
    return periodos.map((dato) => ({
      idRegistro: dato.perIderegistro.perIderegistro, //se actualiza identificador del periodo
  ...
  consultarDatosArea = (idArea) => {
    ...
    const peticiones = [
      ...
      axios.post(RUTAS_API.PARAMETRIZACION.CARGAR_PERIODOS.CONSULTAR_CONCEPTOS, { uniLiquidacion: areaSeleccionada.liqIderegistro.uniLiquidacion }),
    ];
          ...
          this.state.listaDatos.map((dato, index) => {
            return (
              <tr key={dato.idEditar}>
                <td> ( {dato.rangoIni} - {dato.rangoFin} ) </td>
          ...
          <div className='body-section'>
            ...
            <div className='row mt-8'>
              <Combo opciones={this.state.listaAreaPrestacion}
                ...
                extra={
                  { disabled: this.state.estadoCampo, readOnly: this.state.estadoCampo, isSearchable: true }
                }
              />
              <Combo opciones={this.state.listaVariables.sort((a, b) => (a.nombre > b.nombre) ? 1 : -1)}
                propTexto='nombre'
...
GestionCargarVariablesPeriodos.propTypes = {
...
const VistaRedux = connect(mapStateToProps, mapDispatchToProps)(GestionCargarVariablesPeriodos);

export { VistaRedux as RGestionCargarVariablesPeriodos };
