...
class CertificarVariablesTarifas extends Component {
  ...
  obtenerObjeto = (estado) => {
    ...
    const verificar = (Object.keys(lista[0].listaRangos).length === 0) ? false : true;
    if (verificar) {
      for (let index = 0; index < lista.length; index++) {
        ...
        for (let index = 0; index < arreglo.listaRangos.length; index++) {
          ...
          listaFinal.push({
            varcIderegistro: elementoValores.varc_ideregistro,
            perIderegistro: {
            ...
            varcDescripcion: arreglo.observacion,
            varcValor: elementoValores.nuevovalor,
            varcEstado: estado
          });
        }
      }
    ...
    return lista.map(dato => {
      return {
        varcIderegistro: dato.listaValores.varc_ideregistro,
        perIderegistro: {
          smperIderegistro: periodo
        },
        conIderegistro: {
        ...
        varcValor: dato.listaValores.nuevovalor,
  ...
  construirObjetoVariables = (conceptos) => {
    return conceptos.map((dato, index) => {
      return {
        ...
        listaRangos: (dato.listaRangos != null) ? JSON.parse(dato.listaRangos) : [],
        listaValores: (dato.listaValores != null) ? JSON.parse(dato.listaValores) : [],
                    ...
                    return (
                      <Fragment>
                        <tr key={Util.generarIdControl(index + '_2')}>
                          ...
                          <td>{(datoRaco.varc_estado.trim() == 'P') ? 'Pendiente' : ''}</td>
                        </tr>
                      </Fragment>
                    );
                  })
            ...
            lista.map((dato, index) => {
              return (
                <tr key={Util.generarIdControl(index)}>
                  ...
                  <td>{(dato.listaValores.varc_estado.trim() == 'P') ? 'Pendiente' : ''}</td>
                </tr>
              );
            })
          }
