...
app.controller('GeneralSuiController', function ($scope, $http) {
   // $scope.info = {fechaInicial:null,fechaFinal:null,estado:null,municipio:null};
    ...
    $scope.generarReporte = function () {
        ...
        if ($scope.reporte.$valid) {
            $http.post("generarSuiReguladosPdf", $scope.info).then(function (response) {
    ...
    $scope.generarReporte2 = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.reporte.$valid) {
            $http.post("generarSuiNoReguladosPdf", $scope.info).then(function (response) {
                downloadFileDirect(response.data);
            });
        }
    };
    ...
    $scope.generarReporte3 = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.reporte.$valid) {
            $http.post("generarComprobacionSui", $scope.info).then(function (response) {
    ...
    $scope.generarReporte4 = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.reporte.$valid) {
            $http.post("generarReguladosIndustrialSui", $scope.info).then(function (response) {
                downloadFileDirect(response.data);
            });
        }
    };
