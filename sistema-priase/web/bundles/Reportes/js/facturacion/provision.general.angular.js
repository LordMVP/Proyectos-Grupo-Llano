   ...
   // $scope.info = {fechaInicial:null,fechaFinal:null,estado:null,municipio:null};
    ...
    $scope.generarReporte = function () {
        ...
        if ($scope.reporte.$valid) {
            $http.post("generarProvisionGeneral", $scope.info).then(function (response) {
    ...
    $scope.generarReporte2 = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.reporte.$valid) {
            $http.post("generarProvisionVillavo", $scope.info).then(function (response) {
                downloadFile(response.data);
            });
        }
    };
    ...
    $scope.generarReporte3 = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.reporte.$valid) {
            $http.post("generarProvisionVillavoPeriodo", $scope.info).then(function (response) {
                downloadFile(response.data);
            });
        }
    };
    ...
    $scope.generarReporte4 = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.reporte.$valid) {
            $http.post("generarProvisionPeriodoMunicipios", $scope.info).then(function (response) {
                downloadFile(response.data);
            });
        }
    };
