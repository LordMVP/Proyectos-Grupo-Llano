...
app.controller('ventasController', function ($scope, $http, $q, NgTableParams) {
    $scope.total=-1;
    $scope.info = {numero_orden: null, constructora_id: null, contrato: null};
    $http.post("../contratosTable/getColumns").then(function (response) {
        $scope.columnas = response.data.columnas;
    });
    
    function getData(params) {
        $scope.selected =null;
        return $http.post("../contratosTable/getInfo", {count: params.count(), page: params.page(), extra: $scope.info}).then(function (response) {
    ...
    $scope.generarReporte = function () {
        $http.post("generarFormatoFacturaDeVenta",{'numeroOrden':$scope.selected.numero_orden}).then(function (response) {
