...
var aplicarVista = {
    ...
    data: null,
    /** Inicializa el programa para aplicar los cambio de valor de las  financiaciones especiales, y asigna listeners a controles
     * @returns {void}
     **/
    init: function () {
        that = aplicarVista;
        ...
        $('#btnDescargarSdo').on('click', that.descargarSaldosdxd);
        $('#btnEliminarResumen').on('click', aplicarControl.eliminarResumen);
    ...
    subirAdjunto: function () {
        ...
        if ($('#txtContinuar').val() != "SI" && $('#txtContinuar').val() != "si" && $('#txtContinuar').val() != "Si") {
            __dom.lanzarAlerta("Para continuar el Campo" + $('#txtContinuar').parent().find('label').text() + " Debe contener la palabra SI/Si ", __app.mensajes.atencion);
            return;
        }  
        var data = {
            fechapago: $('#txtContinuar').val()
        };
    ...
    /**
     * Consulta el progreso del proceso de cargar cambios de Valor
     * @returns {void}
     */
    consultarProgreso: function () {
        ...
        aplicarControl.consultarProgreso(fxSuccess);
    },
    /**
     * Consulta el resultado de la importanción de cambios de Valor
     * @param {Object} data - Información enviada por el servidor con los cambios de valor cargados correctamente y las que no se pudieron cargarse
     */
    onConsultarResumenCompleto: function (data) {
        if (data.codigoRespuesta > 0) {
            fillTable('tblResumen', 'formatoResumen', data.resumencorrectos, 'Cambios de Valor Cargados Correctamente');
            }  
            if(data.resumenconerrores.length > 0){
                fillTable('tblResumenConProblemas', 'formatoResumenErrores', data.resumenconerrores, 'Cambios de Valor sin cargar');
            }
            return;
    ...
    descargarErrores: function () 
    {
        ...
        aplicarControl.descargarErrores(that.onDescargarErroresCompleto, data); 
    },
    /**
    ...
    onDescargarErroresCompleto: function (data) {
        ...
        if (data.codigoRespuesta == 2 ) 
        {
            ...
            that.descargarArchivo(that.generarTexto(datos), 'erooresDxD.txt');  
            return;
         }  
    },
     ...
     * @returns {void}
     */
    descargarSaldosdxd: function () 
    {
        var data = {
        };
        aplicarControl.descargarSaldos(that.onDescargarSaldosdxdCompleto, data); 
    },
    ...
    */
    onDescargarSaldosdxdCompleto: function (data) {
        if (data.codigoRespuesta == 0 ) 
        {
             __dom.lanzarAlerta(data.mensaje, __app.mensajes.atencion); 
            return;
        ...
        {
            var datos = data.datos ;
            that.descargarArchivo(that.generarTexto(datos), 'Saldos_DxD.txt');  
            return;
         }  
    },
