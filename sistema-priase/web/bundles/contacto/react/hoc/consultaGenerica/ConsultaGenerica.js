...
class ConsultaGenerica extends Component {
  ...
  renderCeldaAcciones = (props) => {
    ...
    if (this.props.esModal) {
      ...
      funcion = (entidad) => {
        this.props.seleccionarEntidad(entidad, this.props.extra);
  ...
  validarFecha = (Fecha) => {
    ...
    if (Fecha !== "" && Fecha !== null) {
      modFecha = new Date (Fecha).toLocaleDateString();
    }
    return modFecha;
  }

  _buscar(params) {
    const url = (typeof this.props.rutaConsulta == 'function') ? this.props.rutaConsulta() : this.props.rutaConsulta;
    axios.post(url, params)
      .then(respuesta => {
        respuesta.data.datos.forEach((element , index) => {
          respuesta.data.datos[index].conFechaCreacion = this.validarFecha(respuesta.data.datos[index].conFechaCreacion);
          respuesta.data.datos[index].terDocexpedicion = this.validarFecha(respuesta.data.datos[index].terDocexpedicion);
          respuesta.data.datos[index].terFecnacimiento = this.validarFecha(respuesta.data.datos[index].terFecnacimiento);
        });
...
ConsultaGenerica.propTypes = {
  ...
  rutaConsulta: PropTypes.any.isRequired,
