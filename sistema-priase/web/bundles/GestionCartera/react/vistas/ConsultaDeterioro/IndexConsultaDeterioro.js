...
import { RVistaFormConsultaDeterioro, RVistaListConsultaDeterioro,  RVistaListResultadosDeterioro, RVistaListExcepcionesInicioDeterioro} from '../index';
import { ACCION, MODULO } from '../../store/actions/TiposAcciones';
import deterioroNiff from '../../store/servicios/DeterioroNiffServicios';


/**
 * Vista Principal del componente de Consulta Deterioro NIFF
 */
class IndexConsultaDeterioro extends Component {
  constructor(props) {
    super(props)
    this.state = {
      showExcepcion:false,
  ...
  render() {
    ...
    return (
      <Contenedor>
        <br />
        <h1>Consulta Información Procesada </h1>
        <div className="customHr">.</div>
        <br /><RVistaFormConsultaDeterioro/>
        <br /><RVistaListConsultaDeterioro/>
      </Contenedor>
    );
  }
  /**
  ...
  */
  componentDidMount() {
    this.props.setOrigenComponente(MODULO.CONSULTA_DETERIORO);
...
IndexConsultaDeterioro.propTypes = {
...
const VistaRedux = connect(mapStateToProps, mapDispatchToProps)(IndexConsultaDeterioro);
export { VistaRedux as RIndexConsultaDeterioro };