var impresionVista = {
    ...
    enviarInformacionPorFormato: function (nombre, venta, suscripcion, tercero, solicitante, firma, modelo) {
        ...
        var datos = {informacion: {
                ...
                telefonotercero: tercero.telefono,
                ...
                diasterminoperiodo: modelo.diasterminoperiodo,
                formato: {}
            }};
        switch (nombre) {
    ...
    imprimirFormatoCondonacion: function(facturas){
        let factura = {
            ...
            fac_vencidas: facturas.length,
            fechafactura: facturas[0].fechafactura,
            segmento: facturas[0].segmento,
            periodo: facturas[0].periodo,
            interestotal: 0,
            condonarinteres: 0
        }
        for (let i = 0; i < facturas.length; i++) {
            factura.interestotal += parseFloat(facturas[i].interestotal);
            factura.condonarinteres += parseFloat(facturas[i].condonarinteres);
        }
        let fac_condonadas = [factura];
        let info = impresionVista.infoFormatoCondonacionBio(fac_condonadas);
        let contenido = {
            ...
            for (let i = 0; i < facturas.length; i++) {
                ...
                rows += "\
                    <tr>\
                        ...
                        <td>" + factura.fac_vencidas + "</td>\
                        <td>" + new Date(factura.fechafactura).toLocaleDateString() + "</td>\
                        <td>" + factura.segmento + "</td>\
                        <td>" + factura.periodo + "</td>\
                        <td>" + parseFloat(factura.interestotal).toFixed(2) + "</td>\
                        <td>" + parseFloat(factura.condonarinteres).toFixed(2) + "</td>\
                    </tr>\
                ";
            }
            return rows;
