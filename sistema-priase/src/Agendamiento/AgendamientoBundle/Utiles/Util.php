...
class Util {
    ...
    public static function validarToken(&$control, $token) {
        ...
        if(count($decoded) > 0){
            ...
            if ($sesion === null || $sesion->get('idusuario') === null) {
                ...
                $sesion->set('pfi_ideregistro', $decoded['pfi_ideregistro']); 
                $sesion->set('emp_ideregistro', $decoded['emp_ideregistro']);
                $sesion->set('idusuario', $decoded['id_usuario']);
                $sesion->set('idUsuario', $decoded['id_usuario']);
                $sesion->set('idempresa', $decoded['emp_ideregistro']);
                $sesion->set('idEmpresa', $decoded['emp_ideregistro']);
                $sesion->set('usuario', "JULIAN POVEDA");
                $sesion->set('empresa', "LLANOGAS SA ESP");
                $sesion->set('logo', "Llanogas.png");
                $sesion->set('idperfil',  $decoded['pfi_ideregistro']); 
                $sesion->set('acc_ideregistro', $decoded['id_acceso']);
                $sesion->set('idAcceso', $decoded['id_acceso']);
                $sesion->set('idacceso', $decoded['id_acceso']);
        ...
        if ($sesion == null || empty($sesion->get('idusuario')) || 
                empty($sesion->get('idempresa'))) {
