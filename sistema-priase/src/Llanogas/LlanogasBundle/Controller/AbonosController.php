...
class AbonosController extends Controller {
    ...
    public function consultarSuscripcionAction() {
        try {
            ...
            $recaudosDelegado = new RecaudosDelegado($this, $sesion);
            $parametrosSuscripcion = Util::validaSuscripcionCarteraNoHomologada($idSuscripcion);
            ...
            if ($estado == 'C') {
                $suscripciones = $recaudosDelegado->getSuscripcionesCarteraCastigada($documento, $idSuscripcion, $codAnterior);
                  ...
                  if($parametrosSuscripcion['carteraAseoNoHomologada']==1)
                        {
                            $idSuscripcion =$parametrosSuscripcion['suscripcion'];
                            $respuesta['idsuscripcioncarteraAseoNoHomologada']=$idSuscripcion ;                             
                            $suscripciones[0] = $recaudosDelegado->getSuscripcionesAbonos($documento, $idSuscripcion, $codAnterior)[0];                                                       
                        } 
                       else 
                        {
    ...
    public function cargarFacturaSuscripcionAction() {
        try {
            ...
            $suscripcionAseoCarteraNoHomologada = $request->get('idsuscripcioncarteraAseoNoHomologada');
            if (empty($suscripcion)) {
                throw new MyException("No se encontró la suscripción", -1);
            }
            $idsSuscripcion = str_replace('"', '', $suscripcion);
            $recaudosDelegado = new RecaudosDelegado($this, $sesion);
            $parametrosSuscripcion = Util::validaSuscripcionCarteraNoHomologada($idsSuscripcion);
            if($parametrosSuscripcion['carteraAseoNoHomologada']==1 ){
                $idsSuscripcion= $parametrosSuscripcion['suscripcion'];                                
            }
            if(!empty($suscripcionAseoCarteraNoHomologada)){
                $idsSuscripcion= $suscripcionAseoCarteraNoHomologada;
                $parametrosSuscripcion['carteraAseoNoHomologada'] =1;
            }
                
            $respuesta = $recaudosDelegado->getFacturasConSaldo($idsSuscripcion,$parametrosSuscripcion['carteraAseoNoHomologada']);
