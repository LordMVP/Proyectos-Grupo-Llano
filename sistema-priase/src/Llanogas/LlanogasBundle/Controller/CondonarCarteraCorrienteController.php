...
class CondonarCarteraCorrienteController extends Controller {
    ...
    public function getFacturasSuscripcionAction() {
        try {
            ...
            $condonarCarteraCorrienteDelegado = new CondonarCarteraCorrienteDelegado($this, $sesion);
            $facturas = $condonarCarteraCorrienteDelegado->obtenerFacturasCarteraCorriente($idSuscripcion, $idsegmento);
    ...
    public function getFacturasIntCorrienteSuscripcionAction() {
        try {
            ...
            $idsegmento = $request->get("idsegmento");
            $condonarCarteraCorrienteDelegado = new CondonarCarteraCorrienteDelegado($this, $sesion);
            $facturas = $condonarCarteraCorrienteDelegado->obtenerFacturasCarteraIntCorriente($idSuscripcion, $idsegmento);
            $respuesta["codigoRespuesta"] = (empty($facturas) ? 0 : 1);
            $respuesta["datos"] = $facturas;
            $respuesta["mensaje"] = (empty($facturas)) ? "Suscripción sin facturas" : "La consulta se realizó correctamente";
    ...
    public function consultarRestriccionAction() {
        try {
            ...
            $idSuscripcion = $request->get('idsuscripcion');
            $opcion_sel = $request->get('opcion');
           
            if ($opcion_sel === "" && $idSuscripcion === "") {
                throw new MyException("Error, no hay parámetros de búsqueda");
            }

            $condonarCarteraCorrienteDelegado = new CondonarCarteraCorrienteDelegado($this, $sesion);
            $response = $condonarCarteraCorrienteDelegado->consultarRestriccion($idSuscripcion, $opcion_sel);
     ...
     * suscripcion
     */
     public function getFacturasCondonadasAction() {
        try {
            $sesion = Util::iniciarSesion($this);
            $request = $this->getRequest();
            ...
            $parametros["idNotaCondonacion"] = $request->get("idNotaCondonacion");

            $condonarCarteraCorrienteDelegado = new CondonarCarteraCorrienteDelegado($this, $sesion);
            $facturas = $condonarCarteraCorrienteDelegado->obtenerFacturasCondonadas($parametros);
            $respuesta["codigoRespuesta"] = (empty($facturas) ? 0 : 1);
            $respuesta["datos"] = $facturas;
            $respuesta["mensaje"] = (empty($facturas)) ? "No se encontraron facturas para la suscripcion y nota generada." : "La consulta se realizó correctamente";
