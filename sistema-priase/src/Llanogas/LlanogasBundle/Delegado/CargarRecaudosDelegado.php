...
class CargarRecaudosDelegado {
	...
	if(strlen(stristr($nombre,".DAT")) > 0 ) $nombreArchivo = str_replace(".DAT", '', $nombre);
        switch ($infoRecaudo['idmediopago']) {
            case RECAUDADOR_CONVC001://Cotrem
	    ...
	    case RECAUDADOR_CONSUC2:
            ...
            case RECAUDADOR_ATH:
                ...
                break;
	    case RECAUDADOR_ATH10081:
                $this->validarNombre($nombreArchivo, "/^Bioag|^BioLlan|^Llano|^Cusiana(([0-9]{8}))/");
                $listaRegistros = $this->procesarLineasATH($listaLineas, $infoRecaudo);
                break;
	    case RECAUDADOR_ATH11181:
                $this->validarNombre($nombreArchivo, "/^Bioag|^BioLlan|^LLANOGAS|^Cusiana(([0-9]{8}))/");
                $listaRegistros = $this->procesarLineasATH($listaLineas, $infoRecaudo);
                break;
	    case RECAUDADOR_ATH1237:
                $this->validarNombre($nombreArchivo, "/^BIOAGRICOB|^BioLlan|^Llano|^Cusiana(([0-9]{8}))/");
                $listaRegistros = $this->procesarLineasATH($listaLineas, $infoRecaudo);
                break;
	    case RECAUDADOR_ATH074:
                $this->validarNombre($nombreArchivo, "/^bioag|^BioLlan|^Llano|^Cusiana(([0-9]{8}))/");
                $listaRegistros = $this->procesarLineasATH($listaLineas, $infoRecaudo);
                break;
            ...
            case RECAUDADOR_REDMULTICOLOR:
                ...
                break;
	    case RECAUDADOR_REDMULTICOLOR_41:
                $this->validarNombre($nombreArchivo, '/^R([0-9]{7})/');
                $listaRegistros = $this->procesarLineasRedMulticolor_41($listaLineas, $infoRecaudo);
                break;
	    case RECAUDADOR_CONVC99RBM:
                $this->validarNombre($nombreArchivo, '/^R([0-9]{7})/');
                $listaRegistros = $this->procesarLineasRedMulticolor($listaLineas, $infoRecaudo);
                break;
	    ...
	    case RECAUDADOR_RECAUDO_OFICINA:
            ...
            case RECAUDADOR_COLPATRIA:
                ...
                break;
            case RECAUDADOR_COLPAINT:
                $this->validarNombre($nombreArchivo, '/^ERM([0-9]{12})/');
                $listaRegistros = $this->procesarLineasColpatriaInternet($listaLineas, $infoRecaudo);
                break;
	    ...
	    case RECAUDADOR_EFECTY:
            ...
            case RECAUDADOR_EMSA:
                $this->validarNombre($nombreArchivo, '/^411BIO_([0-9]{8})|^411_AJUSTE([0-9]{8})/');
            ...
            case RECAUDADOR_BBVA:
                ...
                $listaRegistros = $this->procesarLineasBancoBogota($listaLineas, $infoRecaudo);
                break;
            case RECAUDADOR_AVALPAYCENTER:
                 $this->validarNombre($nombreArchivo, '/^AVPCENTER_([0-9]{8})/');
                 $listaRegistros = $this->procesarLineasAvalPayCenter($listaLineas, $infoRecaudo);
                 break;
    ...
    private function validarInformacionArchivo($infoRecaudo) {
        try {
            $idEmpresa = $this->sesion->get('idempresa');
            $validacion = $this->cargarRecaudosModel->validarInformacionTemporal($idEmpresa, 105, MAXIMO_PAGO_CARGUE_RECAUDOS, $infoRecaudo['idmediopago']);
            if (!empty($validacion)) {
    ...
    private function procesarLineasCotrem(array $listaLineas, $infoRecaudo) {
        ...
        for ($i = 2; $i < count($listaLineas) - 1; $i++) {
            ...
            $registro['idsuscripcion'] = substr($linea, 44, 6) / 1;
    ...
    private function procesarLineasConsuerte(array $listaLineas, $infoRecaudo) {
        ...
        $listaRegistros = array();
        $linea = $listaLineas[0];
        $infoRecaudo['fechapago'] = $infoRecaudo['fechapago']; //substr($linea, 40, 4) . '-' . substr($linea, 44, 2) . '-' . substr($linea, 46, 2);
    ...
    private function procesarLineasRedMulticolor(array $listaLineas, $infoRecaudo) {
        $listaRegistros = array();
	$registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:00';
		...
		for($x = $i; $x < count($listaLineas) - 1 ; $x++){
                $line=$listaLineas[$x];
                if((substr($linea, 4, 8) / 1)==(substr($line, 4, 8) / 1)){		    
                    $registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:0'.$contador;    
		    $contador++;               
                }
            }
            $registro['idsuscripcion'] = substr($linea, 4, 8) / 1;
	    $registro['valor'] = substr($linea, 12, 23) / 1;
            $registro['idmediopago'] = $infoRecaudo['idmediopago'];
            $listaRegistros[] = $registro;
        }
        return $listaRegistros;
     ...
     * @throws MyException
     */
    private function procesarLineasRedMulticolor_41(array $listaLineas, $infoRecaudo) {
        $listaRegistros = array();
	$registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:00';
        for ($i = 1; $i < count($listaLineas) - 1; $i++) {
            $linea = $listaLineas[$i];
		...
		for($x = $i; $x < count($listaLineas) - 1 ; $x++){
            ...
            $registro['idsuscripcion'] = substr($linea, 4, 8) / 1;
            $registro['valor'] = substr($linea, 35, 13) / 1;
            $registro['idmediopago'] = $infoRecaudo['idmediopago'];
            $listaRegistros[] = $registro;
        }
        return $listaRegistros;
	...
	$registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:00';
		...
		for($x = $i; $x < count($listaLineas) - 1 ; $x++){
                $line=$listaLineas[$x];
                if((substr($linea, 5, 7) / 1)==(substr($line, 5, 7) / 1)){		    
                    $registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:0'.$contador;    
		    $contador++;               
                }
            }

            $registro['idsuscripcion'] = substr($linea, 5, 7) / 1; //substr($linea, 4, 8) / 1;
            //$registro['fechapago'] = $infoRecaudo['fechapago'];//'20' . substr($linea, 50, 2) . '-' . substr($linea, 52, 2) . '-' . substr($linea, 54, 2);
            $registro['valor'] = substr($linea, 35, 13) / 1;//substr($linea, 12, 23) / 1; //+ substr($linea, 35, 13) / 1;
            $registro['idmediopago'] = $infoRecaudo['idmediopago'];
            $listaRegistros[] = $registro;
        }
        return $listaRegistros;
     ...
     * @throws MyException
     */
    private function procesarLineasColpatriaInternet(array $listaLineas, $infoRecaudo) {
        $listaRegistros = array();
        for ($i = 2; $i < count($listaLineas) - 2; $i++) {
            $linea = $listaLineas[$i];
            /*if (strlen($linea) != 162) {
    ...
    private function procesarLineasBancoBogota(array $listaLineas, $infoRecaudo) {
        ...
        $listaRegistros = array();
        $linea = $listaLineas[0];
        $registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:00';//substr($linea, 12, 4) . '-' . substr($linea, 16, 2) . '-' . substr($linea, 18, 2);
        for ($i = 2; $i < count($listaLineas) - 2; $i++) {
            $linea = $listaLineas[$i];
	    $contador = 0;
		...
		for($x = $i; $x < count($listaLineas) - 2 ; $x++){
                $line=$listaLineas[$x];
                if((substr($linea, 42, 8) / 1)==(substr($line, 42, 8) / 1)){		    
                    $registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:0'.$contador;    
		    $contador++;               
                }
            }
    ...
    private function procesarLineasAvalPayCenter(array $listaLineas, $infoRecaudo) {
        if (empty($listaLineas)) {
            throw new MyException('Error el archivo no contiene líneas', -1);
        }
        $listaRegistros = array();
        ...
        for ($i = 1; $i < count($listaLineas); $i++) {
            $registroLinea=array();
            $registroLinea=explode(",",$listaLineas[$i]);
            $idsus=explode("	",$registroLinea[6]);
            if (!is_numeric($idsus[0])) {
                throw new MyException('El archivo no tiene el formato esperado (' . $i . ')', -1);
            }
            $registro['idsuscripcion'] = $idsus[0] / 1;            
            $registro['fechapago'] = $infoRecaudo['fechapago'];//$registroLinea[2];
            $registro['valor'] = $registroLinea[3];
            $registro['idmediopago'] = $infoRecaudo['idmediopago'];
            $listaRegistros[] = $registro;
        }
        return $listaRegistros;
	...
	$registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:00';
		...
		for($x = $i; $x < count($listaLineas) - 1 ; $x++){
            ...
            $registro['idsuscripcion'] = substr($linea, 5, 7) / 1;          
    ...
    private function procesarLineasEmsa(array $listaLineas, $infoRecaudo) {
        $listaRegistros = array();
        if (empty($listaLineas)) {
            throw new MyException('Error el archivo est� vac�o');
        }
        $registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:00';
        for ($i = 1; $i < count($listaLineas); $i++) {
            $linea = $listaLineas[$i];
            ...
            $info = explode(";", $linea);
            if (count($info) != 8) {
                throw new MyException("El archivo no tiene el formato esperado error en la l�nea ( $linea ) ", -1);
            }

            for($x = $i ; $x < count($listaLineas) ; $x++){
                $explo=explode(";",$listaLineas[$x]);
                if(in_array($info[3],$explo)){		    
                    $registro['fechapago'] = $infoRecaudo['fechapago'].' '.'00:00:0'.$contador;    
		    $contador++;                
                }
            }
            //$registro['fechapago'] = $info[0] . '-' . $info[1] . '-' . substr($info[4], 0, 2);
            $registro['idsuscripcion'] = $this->genericoModel->consultarInformacionSuscripcionPcodigo(strval($info[3]))['idsuscripcion'];
