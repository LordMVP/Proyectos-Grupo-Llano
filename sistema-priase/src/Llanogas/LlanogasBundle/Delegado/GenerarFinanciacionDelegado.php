...
// <editor-fold desc="suscripcion">  
     ...
     */
    public function consultarFacturasSuscripcionDocumento($idSuscripcion, $idsegmento, $idDocumento, $idtiposdocumentos, $idconceptodescarte) {
        return $this->financiaciones->consultarFacturasPorSuscripcionDocumentoModel($idSuscripcion, $idsegmento, $idDocumento, $idtiposdocumentos,$idconceptodescarte);
    }
    ...
    private function consultarLiquidacionFinanciacion($idsuscripcion) {
        ...
        $segmentos = $this->financiaciones->consultarSegmentosModel($idsuscripcion);
        $respuesta["segmentos"] = $segmentos;
     ...
     */
    public function consultarDocumentosFinanciacion($idsuscripcion, $idtiposdocumentos) {
        $idusuario = $this->sesion->get('idusuario');
        $idempresa = $this->sesion->get('idempresa');
        $documentos = $this->financiaciones->consultarDocumentoModel($idsuscripcion, $idusuario, $idempresa, $idtiposdocumentos);
        return $documentos;
    }

    /**
     * Lista los tipos documentos asociados a un segmento que se encuentre relacionado a las facturas activas
     * @param int $idsuscripcion
     * @param int $idsegmento 
     * @return Array listado tipos de documentos por segmento
     */
    public function consultarTiposDocumentosFinanciacion($idsuscripcion, $idsegmento) {
        $idusuario = $this->sesion->get('idusuario');
        $idempresa = $this->sesion->get('idempresa');
        $tiposdocumentos = $this->financiaciones->consultarTipoDocumentoModel($idsuscripcion, $idsegmento, $idusuario, $idempresa);
...
// <editor-fold desc="Financiacion">  
     ...
     */
    public function consultarLiquidaciones($iddocumento, $idtiposdocumentos) {
        $idempresa = $this->sesion->get('idempresa');
        $respuesta = $this->financiaciones->consultarLiquidacionFinanciacionModel($iddocumento, $idtiposdocumentos, $idempresa);
        if (empty($respuesta)) {
    ...
    private function armarFinanciacion($financiacion) {
        ...
        $armarFinanciacion['documentosObligatorios'] = $financiacion['documentosObligatorios'];
...
// <editor-fold desc="Subida de documentos anexos">  
    ...
    /**
     * Permite consultar la retsricción del monto de acuerdo a liomite parametrizado
     * @param int $porcentaje
     * @param int $monto
     * @return booelan flag
     */
    public function consultarRestriccion($idSuscripcion) {
        $parametros['idsuscripcion'] = $idSuscripcion;
        $parametros['idusuario'] = $this->sesion->get('idusuario');
        $parametros['idempresa'] = $this->sesion->get('idempresa');
        return $this->financiaciones->consultarRestriccionModel($parametros);
    }
