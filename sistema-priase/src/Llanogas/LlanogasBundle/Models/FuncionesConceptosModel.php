...
// Funcion FECF calcula porcentaje por municipio
     ...
     */
       ...
       public function getTAFNA($idSuscripcion){
        ...
        $sql = "  select coalesce ((
                    ...
                    where ha.hafo_estado <> 'Inactivo' order by hd.hdafo_ideregistro desc limit 1  ), 0 ) resultado" ; 
        $resultado = $this->executeQuery($sql, $parametros);
        if (empty($resultado)) {
     ...
     * JLMENDOZA
     */
        public function getCantidadVisitasAforoExtraOrdinario($idsuscripcion){

            $parametros['idsuscripcion'] = $idsuscripcion ; 
            $sql = "  select coalesce ((select  max(hhd.volumen_producido) - sum(hdcva.hdcva_volumenaforo)
            from aseo.hdafo_detaforo hd
            inner join aseo.hafo_aforos ha on
            ha.hafo_ideregistro = hd.hafo_ideregistro
            inner join aseo.hmafv_maestroaforovisitas hmfv on
            hmfv.afo_ideregistro = ha.hafo_ideregistro
            inner join aseo.hdmaf_detallemaestrovisitas hdmf on
            hdmf.hmafv_ideregistro = hmfv.hmafv_ideregistro
            inner join aseo.hdcva_historicodetalleconceptovisitasaforo hdcva on
            hdcva.hdmaf_ideregistro = hdmf.hdmaf_ideregistro
            join lateral(select max(hm.afo_ideregistro) 
            as hafo_ideregistro ,sum(hdcva.hdcva_volumenaforo) 
            as volumen_producido
            from aseo.hafo_aforos ha
            inner join aseo.hmafv_maestroaforovisitas hm on
            hm.afo_ideregistro = ha.hafo_ideafopadre
            inner join aseo.hdmaf_detallemaestrovisitas hdf on
            hdf.hmafv_ideregistro = hm.hmafv_ideregistro
            inner join aseo.hdcva_historicodetalleconceptovisitasaforo hdcva on
            hdcva.hdmaf_ideregistro = hdf.hdmaf_ideregistro 
            group by hdf.hmafv_ideregistro) hhd on ha.hafo_ideafopadre = hhd.hafo_ideregistro
            where hd.dsus_ideregistr = :idsuscripcion and ha.hafo_estado <> 'Inactivo'
            group by hdmf.hmafv_ideregistro), 0 ) resultado" ; 
            $resultado = $this->executeQuery($sql, $parametros);

            if($resultado[0]['resultado'] < 0 ){
                $parametros['idsuscripcion'] = $idsuscripcion ; 
                $sql ="select coalesce ((select max(hdmf.hdmav_consecutivovisita)
                from aseo.hdafo_detaforo hd
                inner join aseo.hafo_aforos ha on
                ha.hafo_ideregistro = hd.hafo_ideregistro
                inner join aseo.hmafv_maestroaforovisitas hmfv on
                ...
                inner join aseo.hdcva_historicodetalleconceptovisitasaforo hdcva on
                hdcva.hdmaf_ideregistro = hdmf.hdmaf_ideregistro
                where hd.dsus_ideregistr = :idsuscripcion and ha.hafo_estado <> 'Inactivo'), 0 ) visitas";
                $resultadoCantidad = $this->executeQuery($sql, $parametros);
                if (empty($resultadoCantidad)) {
                   return 0 ;
                }
                return $resultadoCantidad[0]['visitas'];
            }
            return 0;
        }
