...
class CondonarCarteraCorrienteModel extends AuditoriaServices {
     ...
     * @return type
     */
    public function consultarFacturasCarteraCorriente($idSuscripcion, $idsegmento) {
        ...
        $sql = "SELECT DISTINCT fac.fac_fecha,
                ...
                WHERE
                    ...
                    AND dsus.dsus_ideregistr = :dsus_ideregistr $complementoSeg
	            order by fac.fac_fecha,fac.fac_fecvence,fac.cic_ano";
        	$resultado = $this->executeQuery($sql, $parametros);
        return $resultado;
    }
    ...
    public function consultarMotivosNota() {
        ...
        FROM
            uni_unidad uni
        WHERE
            uni.est_ideregistro =".ESTRUCTURA_NOTA ." and CAST(uni.uni_propiedad ->> 'emp' as INTEGER)=317";
        $resultado = $this->executeQuery($sql, $parametros);
        return $resultado;    
    }
     ...
     * @return type
     */
    public function consultarFacturasCarteraIntCorriente($idSuscripcion, $idsegmento) {
        $parametros["dsus_ideregistr"] = $idSuscripcion;
        $parametros["idsegmento"] = $idsegmento;
        ...
        $sql = "SELECT DISTINCT fac.fac_fecha,
                ...
                WHERE
                    ...
                    AND dsus.dsus_ideregistr = :dsus_ideregistr $complementoSeg
                order by 
                    fac.fac_fecha,fac.fac_fecvence,fac.cic_ano";
        $resultado = $this->executeQuery($sql, $parametros);
        return $resultado;
    }
     ...
     */
    public function consultarRestriccionModel($parametros, $opcion_sel) {
        switch ($opcion_sel) {
            case 'corriente':
                $parametros["opcion"] = UNIDAD_PROGRAMA_INTERES_CORRIENTE;
                break;
            case 'mora':
                $parametros["opcion"] = UNIDAD_PROGRAMA_INTERES_MORA;
                break;
        }
        $parametros["prg_condona"] = PROGRAMA_CONDONAR_CARTERA_CORRIENTE;
        $sql ='SELECT upr.usu_ideregistro, lus.luspu_limiteporcentaje, lus.luspu_limitemonto 
                ...
                WHERE pp.prg_ideregistro=:prg_condona AND pp.uni_ideregistro=:opcion AND lus.emp_ideregistro = :idempresa AND upr.usu_ideregistro = :idusuario';
        return $this->executeQuery($sql, $parametros);
    }

    /**
     * Consulta las facturas que fueron condonadas de una suscripcion.
     * @param array con nota de condonacion, la suscripcion, empresa
     * @return array facturas condonadas.
     */
    public function consultarFacturasCondonadasModel($parametros) {
        $parametros["idDocumentoNotaCondona"] = UNIDAD_NOTA_CONDONA_FACTURA_CORRIENTE;
        $sql ="SELECT ff.fac_numero numerofactura,
                ff.fac_fecha fechafactura,
                case
                    when exists(SELECT fmg.* FROM aseo.fmg_facturacioncarterag fmg where fmg.fac_ideregistro = ff.fac_ideregistro) AND ff.fac_sdoreal > 0
                        then 'Cartera G'
                        else cc.cnre_nombre
                end segmento,
                per.per_nombre periodo,
                ((SELECT ff2.fac_vlrreal FROM fac_factura ff2 WHERE ff2.fac_ideregistro = ff.fac_idepadre)-ff.fac_vlrreal) interestotal,
                ff.fac_vlrreal condonarinteres
            FROM nofa_notfactura nofa
                INNER JOIN fac_factura ff ON nofa.fac_ideregistro = ff.fac_ideregistro
                INNER JOIN per_periodo per ON per.per_ideregistro = ff.per_ideregistro
                inner join sus_suscripcion ss on ff.sus_ideregistro = ss.sus_ideregistro 
                inner join dsus_detsuscrip dd on ff.dsus_ideregistr = dd.dsus_ideregistr 
                inner join cnre_cnvrecaudo cc on ss.cnre_ideregistr = cc.cnre_ideregistr
            WHERE
                nofa.not_ideregistro=:idNotaCondonacion AND ff.dsus_ideregistr =:idSuscripcion 
                AND ff.uni_documento=:idDocumentoNotaCondona AND ff.emp_ideregistro =:idEmpresa
            order by ff.fac_numero desc";
        return $this->executeQuery($sql, $parametros);
    }
}
