...
class RegistrarBarriosModel extends AuditoriaServices {
    ...
    public function getBarrioConsecutivo( $idEmpresa){
        $parametros['idempresa'] = $idEmpresa;

        $sql = "SELECT max(barr.barrio_cod::integer) + 1 as consecutivo  
                FROM barrios barr  
                INNER JOIN	empresas emp on emp.empresa_cod = barrio_codemp 
                WHERE		emp.empresa_sevemp = :idempresa "; 
        return $this->executeQuery($sql, $parametros);  
    ...
    public function insertarBarrio(array $dataBarrio) {       
        ...
        try{
            ...
            $parametros['barrio_nom']	  =	strtoupper($dataBarrio['nombre']);
	    ...
	    $parametros['barrio_zona_riesgo']  ='false';
