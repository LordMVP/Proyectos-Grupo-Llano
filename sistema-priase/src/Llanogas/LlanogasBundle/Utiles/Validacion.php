...
class Validacion {
    ...
    public function __construct($errorsKeys = null) {
        ...
        $this->messagesInit = [
            ...
            "date" => "El campo __FIELD__ no tiene el formato correcto",
            "numeric" => "El (__FIELD__) no es númerico.",
        ];
    }

    public function make($values, $validations, $messages = null) {
        ...
        foreach ($validations as $key => $validate) {
            ...
            if (count($parts) == 1) {
                if (isset($values[$key])) {
                    ...
                    $this->errors[] = $this->getError($key, "required", $messages);
                }
            } else {
                $value = $this;
    ...
    public function validar($parametros, $configuracion, $mensaje = null) {
        $mensaje = empty($mensaje) ? 'No se recibieron los parámetros correctamente.' : $mensaje;
        $this->make($parametros, $configuracion);
        //Se valida que no hallan errores
        if ($this->fails()) {
            throw new \Externo\FinanciacionesBundle\ValidacionExcepcion(-1, $mensaje, $this->getErrors());
        }
    }
    ...
    public function url($field, $value, $messages = null) {
        $rtn = isset($value) ? filter_var($value, FILTER_VALIDATE_URL) : false;
        $messages = isset($messages) ? $messages : $this->messagesInit;
        if (!$rtn && is_array($messages)) {
    ...
    function validateDate($field, $date, $message = null, $format = 'Y/m/d') {
        if (empty(trim($date))) {
            return;
        }
        ...
        $messages = isset($messages) ? $messages : $this->messagesInit;
        if (!$result) {
            $this->errors[] = $this->getError($field, "date", $messages);
        }
    }
