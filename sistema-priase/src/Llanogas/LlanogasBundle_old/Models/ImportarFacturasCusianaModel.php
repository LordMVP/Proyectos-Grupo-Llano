 ...
 * @author mebonilla
 */
class ImportarFacturasCusianaModel extends AuditoriaServices {
    ...
    public function insertarFactura($factura) {
        ...
        $this->setCampo($factura, $parametros, "uni_tipusosuscr", "uni_tipusosuscr");
        ...
        $this->setCampo($factura, $parametros, "fac_vato", "fac_sdoreal");
        ...
        $this->setCampo($factura, $parametros, "fac_vato", "fac_vlrreal");
        ...
        $this->setCampo($factura, $parametros, "felec_sistema", "felec_sistema");        
        $this->setCampo($factura, $parametros, "felec_idefactura", "felec_idefactura");        
        $this->setCampo($factura, $parametros, "fac_fech", "fac_fech");        
        $this->setCampo($factura, $parametros, "fac_fecf", "fac_fecf");        
        $this->setCampo($factura, $parametros, "fac_vato", "fac_vato");        
        $this->setCampo($factura, $parametros, "fac_desc", "fac_desc");        
        $this->setCampo($factura, $parametros, "cli_noco", "cli_noco");        
        $this->setCampo($factura, $parametros, "tipopersona", "tipopersona");        
        $this->setCampo($factura, $parametros, "cli_coda", "cli_coda");        
        $this->setCampo($factura, $parametros, "cli_nomb", "cli_nomb");        
        $this->setCampo($factura, $parametros, "cli_apel", "cli_apel");        
        $this->setCampo($factura, $parametros, "aplicafel", "aplicafel");        
        $this->setCampo($factura, $parametros, "dcl_mail", "dcl_mail");        
        $this->setCampo($factura, $parametros, "arb_nomb", "arb_nomb");        
        $this->setCampo($factura, $parametros, "dcl_dire", "dcl_dire");        
        $this->setCampo($factura, $parametros, "dcl_ntel", "dcl_ntel");        
        $this->setCampo($factura, $parametros, "mediopago", "mediopago");        
        $this->setCampo($factura, $parametros, "nitfacturadorelectro", "nitfacturadorelectro");        
        $this->setCampo($factura, $parametros, "idusuario", "idusuario");        
        $this->setCampo($factura, $parametros, "ordendeservicio", "ordendeservicio");        
        $this->setCampo($factura, $parametros, "tipocompra", "tipocompra");        
        $this->setCampo($factura, $parametros, "observaciones", "observaciones");    
        try{
          $resultado =  $this->insertar($parametros, "fcfac_felecusfactura", "sq_fcfac_ideregistro");
        } catch (\Exception $ex) {
    ...
    public function insertarDetalleFactura($detalle) {
        ...
        try{
           $resultado=$this->insertar($parametros, "fcdfac_felecusdetfactura", "sq_fcdfac_ideregistr"); 
        } catch (\Exception $ex) {
            throw  new MyException('Error, Verifique los conceptos Parametrizables'+ $ex,-1);
        }
        return $resultado;
    }
     ...
     * periodo activo de la suspensiond
    ...
    public function leerTablaTemporal($idproceso,$idempresa,$idusuario){
        ...
        $parametros['idempresagas'] = LLANOGAS_IDPROYCTO;
        
        ///  incluir un inner join para la con el dsus_det
        $sql="SELECT   *
            FROM      temp_impfactura_enc_cusiana   where ipfe_estado = 'A' and idproceso = :idproceso and emp_ideregistro = :idempresa
            LIMIT 500" ;
         $resultado = $this->executeQuery($sql, $parametros);
     ...
     public function existeTablaTempEncabezado() {
        ...
        $sql = "SELECT count(*) cantidadtablas
                ...
                WHERE TABLE_NAME = ' temp_impfactura_enc_cusiana ';";
    ...
    public function  vaciarTablaTempEncabezado($idEmpresa, $idusuario){
        try {
            $sql = "DELETE FROM  temp_impfactura_enc_cusiana _cusiana ";
    ...
    public function crearTablaTempEncabezado() {
        $sql="CREATE TABLE  temp_impfactura_enc_cusiana 
                (
                  ...
                  CONSTRAINT  temp_impfactura_enc_cusiana_pkey PRIMARY KEY (ipfe_ideregistr))";
        $this->executeQuery($sql);
         $sqlIndxPcodigoBio = 'CREATE INDEX "IX_imfe_dsus_pcodigo_bio"  ON  temp_impfactura_enc_cusiana  USING btree  (dsus_pcodigo_bio COLLATE pg_catalog."default")';
        $this->executeQuery($sqlIndxPcodigoBio);        
        $sqlIndxPcodigoGas = 'CREATE INDEX "IX_imfe_dsus_pcodigo_gas"  ON  temp_impfactura_enc_cusiana   USING btree  (dsus_pcodigo_gas COLLATE pg_catalog."default")';
        $this->executeQuery($sqlIndxPcodigoGas);        
        $sqlIndxEmpIderegistro = 'CREATE INDEX "IX_imfe_emp_ideregistro"  ON  temp_impfactura_enc_cusiana   USING btree  (emp_ideregistro)';
        $this->executeQuery($sqlIndxEmpIderegistro);        
        $sqlIndxIdUsuario = 'CREATE INDEX "IX_imfe_usu_ideregistro"  ON  temp_impfactura_enc_cusiana   USING btree  (usu_ideregistro)';
        $this->executeQuery($sqlIndxIdUsuario);
        $sqlIndxEstado = 'CREATE INDEX "IX_imfe_estado"  ON  temp_impfactura_enc_cusiana   USING btree  (ipfe_estado)';
    ...
    public function insertarFacturaEncabezado($factura) {
        ...
        $this->setCampo($factura, $parametros, "emp_ideregistro", "emp_ideregistro");
        $this->setCampo($factura, $parametros, "felec_sistema", "felec_sistema");
        $this->setCampo($factura, $parametros, "felec_idefactura", "felec_idefactura");
        $this->setCampo($factura, $parametros, "uni_documento", "uni_documento");
        $this->setCampo($factura, $parametros, "uni_tipdocument", "uni_tipdocument");
        $this->setCampo($factura, $parametros, "uni_tipusosuscr", "uni_tipusosuscr");
        $this->setCampo($factura, $parametros, "fac_fech", "fac_fech");
        $this->setCampo($factura, $parametros, "fac_fecf", "fac_fecf");
        $this->setCampo($factura, $parametros, "fac_vato", "fac_vato");
        ...
        $this->setCampo($factura, $parametros, "mediopago", "mediopago");
        $this->setCampo($factura, $parametros, "nitfacturadorelectro", "nitfacturadorelectro");
        $this->setCampo($factura, $parametros, "idusuariocus", "idusuario");
        $this->setCampo($factura, $parametros, "ordendeservicio", "ordendeservicio");
        $this->setCampo($factura, $parametros, "tipocompra", "tipocompra");
        $this->setCampo($factura, $parametros, "observaciones", "observaciones");
        ...
        try{
          $resultado =  $this->insertar($parametros, " temp_impfactura_enc_cusiana ", "sq_ipfe_ideregistr_cus");
        } catch (\Exception $ex) {
    ...
    public function validaEstadoGasModelo($idusuario, $idProceso){
        ...
        $parametros['idusuario'] = $idusuario;
        $parametros['idproceso'] = $idProceso;
        $sql="update  temp_impfactura_enc_cusiana  set ipfe_estado ='E', ipfe_mensaje ='Suscripcion Gas No esta Activa' 
                FROM (
                    SELECT   impf.ipfe_ideregistr  idtemp
                    FROM 				 temp_impfactura_enc_cusiana  impf
                    INNER JOIN 	dsus_detsuscrip dsusgas on dsusgas.dsus_pcodigo = impf.dsus_pcodigo_gas and dsusgas.emp_ideregistro = 322
                    WHERE       dsusgas.dsus_estado not in ('A')  and impf.usu_ideregistro =:idusuario and  impf.idproceso =:idproceso 
                )as info
                WHERE 			info.idtemp =  temp_impfactura_enc_cusiana.ipfe_ideregistr";
        $this->executeQuery($sql, $parametros);
    }
    ...
    public function UsuarioGasNoExistenPrisma($idusuario, $idProceso){
        ...
        $parametros['idusuario'] = $idusuario;
        $parametros['idproceso'] = $idProceso;
        $sql="update  temp_impfactura_enc_cusiana  set ipfe_estado ='E', ipfe_mensaje ='Suscripcion Gas no Existe en Prisma' 
                FROM (
                select impf.ipfe_ideregistr idtemp
                from  temp_impfactura_enc_cusiana  impf 
    ...
    public function buscaSuscripcionesHomologar($idusuario, $idProceso){       
        ...
        $parametros['idproceso'] = $idProceso;
         $sql="SELECT
                ...
                FROM
                                                     temp_impfactura_enc_cusiana  impf
    ...
    public function actualizarRegistroProceso($idTemporal, $estado, $mensaje) {
        ...
        $this->actualizar($parametros, 'temp_impfactura_enc_cusiana', 'ipfe_ideregistr=:ipfe_ideregistr');
    }
    
    public function validaEmpresaSuscripcionSession($idEmpresa, $idProceso, $idusuario){
       
        $sql="update  temp_impfactura_enc_cusiana  set ipfe_estado ='E', ipfe_mensaje ='Suscripcion no pertenece a la empresa de la Sesion' 
                FROM (
                    select impt.ipfe_ideregistr  idtemp from  temp_impfactura_enc_cusiana  impt
    ...
    public function validarCicloSeleccionado($cicloSelecionado, $idUsuario, $idEmpresa, $idProceso){
        ...
        $parametros['idproceso'] = $idProceso;
        $parametros['idempresa'] = $idEmpresa;
        $sql="update  temp_impfactura_enc_cusiana  set ipfe_estado ='E', ipfe_mensaje ='Error, La suscripción no pertenece al ciclo Seleccionado' 
                FROM (
                    SELECT   impf.ipfe_ideregistr  idtemp 
                    from 				 temp_impfactura_enc_cusiana  impf
                    INNER JOIN 	dsus_detsuscrip  dsusgas ON dsusgas.dsus_pcodigo = impf.dsus_pcodigo_gas
    ...
    public function validarCiclosActivos( $idUsuario, $idEmpresa, $idProceso){
        ...
        $parametros['idproceso'] = $idProceso;
        $parametros['idempresa'] = $idEmpresa;
        $sql="update  temp_impfactura_enc_cusiana  set ipfe_estado ='E', ipfe_mensaje ='No se encontró el ciclo y periodo para la suscripción' 
                FROM (
                    SELECT   impf.ipfe_ideregistr  idtemp 
                    from 				 temp_impfactura_enc_cusiana  impf
                    INNER JOIN 	dsus_detsuscrip  dsusgas ON dsusgas.dsus_pcodigo = impf.dsus_pcodigo_gas
                    ...
                    AND  dsusgas.cic_ideregistro not in (SELECT   cic.cic_ideregistro
                                                        from 		 temp_impfactura_enc_cusiana  impf1
     ...
     public function consultarResumen($idUsuario, $idEmpresa) {
        ...
        $sql = "select  'I' tipo, 
                impf.ipfe_mensaje mensaje,
                 (SELECT COUNT(*) FROM  temp_impfactura_enc_cusiana   impfi WHERE impfi.ipfe_estado = 'I'  AND  impfi.usu_ideregistro = $idUsuario and impfi.emp_ideregistro = $idEmpresa) cantidad_facturas/*Facturas_procesadas*/,
                (SELECT trunc(sum(impfs.fac_vato)) FROM  temp_impfactura_enc_cusiana  impfs where impfs.ipfe_estado ='I' AND  impfs.usu_ideregistro = $idUsuario and impfs.emp_ideregistro = $idEmpresa) total_facturacion_cargada
                 from  temp_impfactura_enc_cusiana  impf 
                ...
                UNION ALL
                    ...
                    'Facturas con errores' mensaje,
                     (SELECT COUNT(*) FROM  temp_impfactura_enc_cusiana   impfi WHERE impfi.ipfe_estado = 'E' AND  impfi.usu_ideregistro = $idUsuario and impfi.emp_ideregistro = $idEmpresa) cantidad_facturas/*Facturas_procesadas*/,
                    (SELECT trunc(sum(impfs.fac_vato)) FROM  temp_impfactura_enc_cusiana  impfs where impfs.ipfe_estado ='E' AND  impfs.usu_ideregistro = $idUsuario and impfs.emp_ideregistro = $idEmpresa) total_facturacion_cargada
                     from  temp_impfactura_enc_cusiana  impf 
                ...
                UNION ALL
                    ...
                    select  'U' tipo, 
                    impf.ipfe_mensaje mensaje,
                     (SELECT COUNT(*) FROM  temp_impfactura_enc_cusiana   impfi WHERE impfi.ipfe_estado = 'U' AND  impfi.usu_ideregistro = $idUsuario and impfi.emp_ideregistro = $idEmpresa) cantidad_facturas/*Facturas_procesadas*/,
                    (SELECT trunc(sum(impfs.fac_vato)) FROM  temp_impfactura_enc_cusiana  impfs where impfs.ipfe_estado ='U' AND  impfs.usu_ideregistro = $idUsuario and impfs.emp_ideregistro = $idEmpresa) total_facturacion_cargada
                     from  temp_impfactura_enc_cusiana  impf 
    ...
    public function consultarResumenErrores($idUsuario, $idEmpresa) {
     ...
     public function cancelarFacturacionActionUser($idUsuario, $idEmpresa ) {       
         $sql = "UPDATE   	 temp_impfactura_enc_cusiana  
    ...
    public function actualizaClienteDesHomologadoDefenitivo($idUsuario, $idEmpresa, $idProceso ) {       
         $sql = "UPDATE  temp_impfactura_enc_cusiana  set ipfe_mensaje ='Factura no Procesada, Cliente desHomologado definitivo', ipfe_estado ='E'
                    from (
                        select  dsusbio.dsus_ideregistr idsuscripcionbio, dsusgas.sus_ideregistro idsuscriptorgas,
                    ...
                    WHERE info.idtemporal =  temp_impfactura_enc_cusiana.ipfe_ideregistr";
        return $this->executeQuery($sql); 
    }
    /**
    ...
    public function getFacturasSinNumero($idEmpresa) {
        $parametros["idempresa"] = $idEmpresa;
        $sql = "select * from fcfac_felecusfactura where  fac_numero is null and fac_estado = 'T'  and emp_ideregistro = :idempresa";
        $resultado = $this->executeQuery($sql, $parametros);
        return $resultado;
    }
}
