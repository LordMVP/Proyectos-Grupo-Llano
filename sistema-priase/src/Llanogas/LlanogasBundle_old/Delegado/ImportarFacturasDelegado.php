...
use Llanogas\LlanogasBundle\Utiles\InterpreteXml;
 ...
 * @author mebonilla9
 */
class ImportarFacturasDelegado {
    ...
    private function obtenerOtrasEmpresas() {
        $empresas = $this->importarFacturasModel->consultarOtrasEmpresas();
        if (empty($empresas)) {
    ...
    public function importarFacturas($listaArchivos, $cicloSeleccionado) {
        ...
        try {
            ...
            $interprete = new InterpreteXml($this);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////   
    ...
    public function ActualizaFacturasPeriodoAnterior($cicloSeleccionado) {
        ...
        if ($this->sesion->get('idempresa') == CUSIANAGAS_IDPROYECTO) {
            throw new MyException(' Debe iniciar sesión con una empresa Diferente a  ' . $this->sesion->get('empresa'));
        }
    ...
    public function getTipo() {
        ...
        $idUsuario = $this->sesion->get('idusuario');
        return $this->genericoModel->getInformacionUnidadPorClase(CLA_TIPO_AFECTACION, $idEmpresa, PROGRAMA_IMPORTAR_FACTURA_BIO_ACE, $idUsuario);
    }
    ...
    public function cargarInformacionTemporal($factura, $contador) {
          ...
          $factura["pcodigobio"] = $factura["pcodigo"];
            $factura["facnumero"] = $factura["numfactura"];
            if (array_key_exists('tipouso', $factura)) {
                $factura['tipouso'] = $factura["tipouso"];
            }
            $factura["facfecha"] = $factura["facfecha"];
            $factura["facfecvence"] = $factura["facfecvence"];
            if ($this->sesion->get('idempresa') == 300) {
                $factura["facfecvence"] = 'now()';
            }
            $factura["facsdoreal"] = $factura["facsdoreal"];
            $factura["pcodigogas"] = $factura["suscripcion"];
            //  OJO verificar la logica a  aplicar en el hilo
            if (array_key_exists('conceptofinan', $factura)) {
                if ($factura["conceptofinan"] <> null || $factura["conceptofinan"] <> '') {
                    $factura["concepto_bio"] = $factura["conceptofinan"];
                }
            }
            ...
            $idEncabezado = $this->importarFacturasModel->insertarFacturaEncabezado($factura);
    ...
    public function existeTablaTemporalEncabezado() {
        try {
            $idEmpresa = $this->sesion->get('idempresa');
            $idusuario = $this->sesion->get('idusuario');
            $tablaExiste = $this->importarFacturasModel->existeTablaTempEncabezado();
            if ($tablaExiste > 0) {
                $this->importarFacturasModel->vaciarTablaTempEncabezado($idEmpresa, $idusuario);
                return;
            }
            $this->importarFacturasModel->crearTablaTempEncabezado();
        } catch (\Exception $exc) {
    ...
    public function consultarResumen() {
         $idEmpresa = $this->sesion->get('idempresa');
        $idUsuario = $this->sesion->get('idusuario');
        $resultado['resumencorrectos'] = $this->importarFacturasModel->consultarResumen($idUsuario, $idEmpresa);
        $resultado['resumenconerrores'] = $this->importarFacturasModel->consultarResumenErrores($idUsuario, $idEmpresa);
        return $resultado;
    }
