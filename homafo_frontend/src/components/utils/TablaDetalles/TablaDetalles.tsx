...
interface IProps{
    valorkey:any,
    datos:any[],
    detalles:(value : any)=>void,
    encabezado:string[]
}

class TablaDetalle extends React.Component<IProps,any>
{
    ...
    {
        //console.log(cell);
        return <Button variant="success" key={cell} onClick={ this.seleccion.bind(this,row)}>Informacion</Button>;
    }
        ...
        return(
            <div className="table-responsive">
                        ...
                        <BootstrapTable data-mobile-responsive="true" wrapperClasses="table" data={this.props.datos} striped={true} hover={true} pagination={paginationFactory({})} keyField={this.props.valorkey} noDataIndication="No Hay Informacion...">                                
                                        {this.getEncabezado()}
                                        <TableHeaderColumn dataAlign="center" dataField="button" dataFormat={this.formatoBotton}>Informacion</TableHeaderColumn> 
                        </BootstrapTable>          
            </div>
        );
    }
