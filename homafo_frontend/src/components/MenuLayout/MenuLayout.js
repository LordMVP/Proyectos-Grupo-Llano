...
//import Loader from '../loader/Init';
...
class MenuLayout extends Component {
  ...
  */
  ...
  obtenerTituloPrograma = () => {
    const ruta = this.props.location.pathname.substring(0);
    ...
    for (const llave in this.props.rutasVista) {
      ...
      const pathExpr = new RegExp(vista.url + "\/?.*");
      if (pathExpr.exec(ruta) && llave !== "RAIZ" && vista.menu === false) {
        return '';
      }
    }
  ...
  obtenerRouter = (programa) => {
    ...
    //console.log("Programa localiza: "+prgLocaliza);
    //const ruta = prgLocaliza.substring(prgLocaliza.lastIndexOf('/'));
    //console.log("Ruta en react "+ruta)
    let rutasMatch = [];
    for (const llave in this.props.rutasVista) {
      //console.log("Llave "+llave);
      const vista = this.props.rutasVista[llave];
      //console.log("Validando "+vista.url)
      //console.log("Validando 2"+PARAMETROS.BASENAME+vista.url);
      const pathExpr = new RegExp(prgLocaliza + "\/?.*");
      if (pathExpr.exec(PARAMETROS.BASENAME + vista.url) && llave !== "RAIZ") {
       // console.log("Ruta valida " + vista.url);
        rutasMatch.push
          (
            <Route
              path={vista.url}
  ...
  render() {
    ...
    let rutas = (
      <Suspense fallback={<NewLoader />}>
        <Switch>
          <Route path={rutasVista.RAIZ.url} exact component={rutasVista.RAIZ.componente} titulo={titulo} />
          {this.obtenerProgramasPermitidos()}
          <Route component={rutasVista.VISTA_NO_PERMITIDA.componente} />
        </Switch>
      </Suspense>
    );

    return (
      <Layout menu={opcionesMenu} tituloPrograma={this.obtenerTituloPrograma()} sesion={sesion}>
        {rutas}
      </Layout>
    )
  }
