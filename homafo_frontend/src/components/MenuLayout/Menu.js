...
class Menu extends Component {
  ...
  renderItemMenu = (opcion) => {
    ...
    if (
      !opcion.menuItem &&
      opcion.prgIderegistro &&
      opcion.prgIderegistro.prgLocaliza
    ) {
      if (opcion.visible === false) {
        return null;
      }
      ...
      let link = null;
      if (
        opcion.opcTipo === TIPOS_MENU.AFOROS ||
        opcion.opcTipo === TIPOS_MENU.HOMOLOGACIONES
      ) {
        let rutaPrograma = opcion.prgIderegistro.prgLocaliza;
        rutaPrograma = rutaPrograma.replace(PARAMETROS.BASENAME, "");
        link = (
          <Link
            to={{
              pathname: rutaPrograma,
              state: { programa: opcion.prgIderegistro },
            }}
  ...
  mostrarOcultarMenu = (e) => {
    ...
    if (menu && menu.classList && menu.classList.contains("open")) {
      menu.classList.remove("open");
      return;
    } else if (menu && menu.classList) {
    ...
    return (
      <div className="menu">
        ...
        <div className="menu__arbol">
          <ul className="menu__arbol__opciones">{this.renderMenu()}</ul>
        </div>
