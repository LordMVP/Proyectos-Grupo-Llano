 ...
 * 
 * @author TSI
 * Clase con los métodos donde se aplica la lógica para Estado Cartera
 */
@Service
public class EstadoCarteraServiciosImpl implements EstadoCarteraServicios {

	@Autowired
	EstadoCarteraRepositorio _repositorio;
	//@Autowired
	...
	/**
	 * @param Long id
	 * Método para busqueda de Estado Cartera por id
	 * @return Estado Cartera
	 */
	@Override
	public EstadoCartera findById(Long id) {
		Optional<EstadoCartera> item = _repositorio.findById(id);
		if (!item.isPresent()) {
...
//		}
	...
	/**
	 * 
	 * Método para listar todos los Estado Cartera
	 * @return lista Estado Carteras
	 */
	@Override
	public List<EstadoCartera> findByAll() {
		List<EstadoCartera> items = (List<EstadoCartera>)_repositorio.findAll();
		if (items.size() == 0){
			throw new NoDataFoundException();
		}
...
//		}
	...
	/**
	 * 
	 * Método para listar todos los Estado Cartera por empresa
	 * @return lista Estado Carteras
	 */
	@Override
	public List<EstadoCartera> findByEmpresasevemp(Long idEmpresa) {
		List<EstadoCartera> items = (List<EstadoCartera>)_repositorio.findByEmpresasevemp(idEmpresa);
		if (items.size() == 0){
			throw new NoDataFoundException();
		}
//		String nombresectores = "";
...
//		}
	...
	/**
	 * 
	 * Método para guardar EstadoCartera 
	 * 
	 * @return registro creado
	 */
	@Override
	public EstadoCartera save(EstadoCartera item) {
		EstadoCartera estadoCartera = _repositorio.save(item);
