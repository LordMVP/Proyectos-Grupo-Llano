 ...
 * 
 * @author TSI
 * Clase con los métodos donde se aplica la lógica para Edad Cartera
 */
@Service
public class EdadCarteraServiciosImpl implements EdadCarteraServicios {

	@Autowired
	EdadCarteraRepositorio _repositorio;
	
	/**
	 * @param Long id
	 * Método para busqueda de Edad Cartera por id
	 * @return Edad Cartera
	 */
	@Override
	public EdadCartera findById(Long id) {
		Optional<EdadCartera> item = _repositorio.findById(id);
		if (!item.isPresent()) {
...
//		}
	...
	/**
	 * 
	 * Método para listar todos los Edad Cartera
	 * @return lista Edad Carteras
	 */
	@Override
	public List<EdadCartera> findByAll() {
		List<EdadCartera> items = (List<EdadCartera>)_repositorio.findAll();
		if (items.size() == 0){
			throw new NoDataFoundException();
		}
...
//		}
	...
	/**
	 * 
	 * Método para listar todos los Edad Cartera por empresa
	 * @return lista Edad Carteras
	 */
	@Override
	public List<EdadCartera> findByEmpresasevemp(Long idEmpresa) {
		List<EdadCartera> items = (List<EdadCartera>)_repositorio.findByEmpresasevemp(idEmpresa);
		if (items.size() == 0){
			throw new NoDataFoundException();
		}
//		String nombresectores = "";
...
//		}
	...
	/**
	 * 
	 * Método para guardar Edad Cartera 
	 * 
	 * @return registro creado
	 */
	@Override
	public EdadCartera save(EdadCartera item) {
		EdadCartera edadCartera = _repositorio.save(item);
