...
!function($) {
  ...
  $.fn.tree = function(options) {
    ...
    $.fn.click_event = function() {
      ...
      if( $(this).hasClass('jtree-arrow') || $(this).prev().hasClass('jtree-arrow') ) {
        button = $(this).hasClass('jtree-arrow') ? $(this) : $(this).prev();
        button[button.hasClass('open') ? 'close' : 'open'](o.animation);
        if(o.only_one) {
          button.closest('li').siblings().children('span.jtree-arrow').close(o.animation);
        }
      }

      //recalcular el tamaño
      var linksAbiertos = $('ul[style="display: block;"] a');
      var maxWidth = 0;
      $.each(linksAbiertos, function(a, anchor){
        var w = $(anchor).text().length*3.2;
        if (w>maxWidth) {
            maxWidth = w;
        }
      });
      $('#menuItems').width( 180+maxWidth   );
    ...
    for(var i = 0; i < this.length; i++) {
      if(this[i].tagName === 'UL') {
        ...
        $(this[i]).find('li > span.jtree-arrow, li > span.jtree-arrow + a').on('click', {tree : this[i]}, function(e) {
          $(this).click_event();
