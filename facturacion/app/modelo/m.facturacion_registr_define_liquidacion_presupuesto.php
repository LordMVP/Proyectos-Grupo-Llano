...
class m_facturacion_registr_define_liquidacion_presupuesto extends database {
	...
	public function guardar($post){
        ...
        for ($i=0; $i<count($resp); $i++) {
            ...
            $consulta="insert into codo_condocumen (" . $campos . ",cue_idedebito,cue_tardebito,cue_idecredito,cue_tarcredito,codo_porcentaje) values (" . $valores . ",NULL,NULL," . $resp1[0] . ",'" . $resp1[2] . "','" . $resp1[4] . "') RETURNING codo_ideregistr";
             ///*echo $consulta*/;
	...
	public function consultar($post){
		...
		switch ($post["accion_m"]){
			case 'concepto_prin':
				...
				break;
			case 'Cuentas':
				$consulta='select "cue_ideregistro","cue_ideregistro","cue_tarcodi","cue_codigo","cue_nombre" FROM "cue_cuenta" WHERE cue_tarcodi=\'14\''; 
				break;
			...
			case 'tablaDoc':
				$consulta="select codo.codo_ideregistr
							...
							inner join liq_liquidacion liq on codo.uni_liquidacion = liq.uni_liquidacion where codo.uni_concepto='" . $post['uni_concepto'] . "' and codo.cue_tarcredito=14"; 
				break;
			}			
		///*echo $consulta*/;
		$this->conecta_db();
