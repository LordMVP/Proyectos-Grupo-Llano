...
var cargarEventos=function(){	
	...
	$('#getContab').on('click',function(){window.open('programa.php?modulo=facturacion_registr_define_liquidacion_contabiliza','_self')}).css({'background' : '#CADCFE','color' : '#0066CC'});
    ...
    $('#btAdicionar').on('click',function(){
		...
		var t1=$('<tr>').html('<td>'+ selId +'</td><td>'+ selTarCodi +'</td><td>'+ selCuenta +'</td><td></td><td><input type="text" value="100" size="3"/></td>');
		if($('#ndebcre').val()==='C'){
			t1=$('<tr>').html('<td>'+ selId +'</td><td>'+ selTarCodi +'</td><td></td><td>'+ selCuenta +'</td> <td><input type="text" value="100" size="3"/></td>');
			}
			//$("#divRespuesta").html('Selecciono Cuenta.'+ selCuenta +' '+$('#ndebcre').val() );
	...
	$('#CuentaSelect tr').each(function () {
		...
		if (!isNaN(pk)){
			...
			if($(this).find("td").eq(3).html()){			
				totalCre+=+cPor;
				ctasGraba+=":"+'NULL;0;'+pk+';'+cTar+';'+cPor;
				}
			}
		});
	if(totalDeb!=totalCre || totalDeb!=100 || totalCre!=100){
        msgError+="...Debitos o Creditos Diferentes al 100%";	
		}
	$('#divRespuesta').html('*TOTAL Deb:'+totalDeb+' Cre:'+totalCre +' '+msgError);	
	if(msgError==="")
	   return true;
	else
	   return false;
