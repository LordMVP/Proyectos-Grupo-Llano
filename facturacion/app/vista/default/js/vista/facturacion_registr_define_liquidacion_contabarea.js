...
var cargarEventos=function(){
	...
	$('#opEditar').on('click',function(){if ($('#uni_concepto').val().trim()===''){alert('Por favor seleccione un registro para poder editarlo.');setTimeout(function(){$('#opCancelar').trigger('click');},20);
			return false;};	cconcurr.br('Yw0K','cots_contipsusc',$('#uni_concepto').val());});
	$('#getContab').on('click',function(){window.open('programa.php?modulo=facturacion_registr_define_liquidacion_contabiliza','_self')});
	$('#getArea').on('click',function(){window.open('programa.php?modulo=facturacion_registr_define_liquidacion_contabarea','_self')}).css({'background' : '#CADCFE','color' : '#0066CC'});
...
var bloqueaInput=function(opcion){
	...
	$('#uni_tipsuscripc').attr("disabled",opcion);
...
/*
<!--
Tabla cots_contipsusc  uni_tipsuscripc,uni_concepto,emp_ideregistro,cue_ideregistro,cue_tarcodi,cots_porcentaje
